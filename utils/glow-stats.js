...
async function fetchPolSummary() {
  const base =
    process.env.PONDER_API_BASE ||
    "https://glow-ponder-listener-2-production.up.railway.app/";
  const url = new URL("/pol/summary", base);
  const response = await axios.get(url.toString());
  const payload = response?.data ?? {};
  return {
    totalEmbeddedLiquidity:
      payload?.total?.breakdown?.usdg !== undefined &&
      payload?.total?.breakdown?.usdg !== null
        ? Number(formatUnits(BigInt(payload.total.breakdown.usdg), USDG_DECIMALS))
        : null,
  };
}
...
async function fetchGlowStats() {
  ...
  const sources = {
    ...
    polSummary: { label: "PoL summary", required: false },
  ...
  const vaultBalanceResult = results[1];
  const marketStatsResult = results[2];
  const polSummaryResult = results[3];
  ...
  if (polSummaryResult.status !== "fulfilled") {
    recordFailure("polSummary", polSummaryResult.reason);
  }
  ...
  const marketStats =
    marketStatsResult.status === "fulfilled"
      ...
      : null;
  const polSummary =
    polSummaryResult.status === "fulfilled"
      ? polSummaryResult.value || null
      : null;
  const allData =
  ...
  const totalEmbeddedLiquidity = polSummary?.totalEmbeddedLiquidity ?? null;
  ...
  return {
    stats: {
      ...
      totalEmbeddedLiquidity:
        typeof totalEmbeddedLiquidity === "number"
          ? totalEmbeddedLiquidity
          : null,
